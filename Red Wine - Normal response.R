...
hist_plot <- ggplot(y_summary) + geom_bar(aes(x=y), fill = "#4B7599", color = "#4B7599") +
  ggtitle('Red wine quality scores histogram')+ theme_classic() +
  scale_x_continuous(breaks = seq(y[which.min(y)],y[which.max(y)],1))
hist_plot

# Saving it
ggsave(filename = paste('plots/Red wine quality scores histogram.png'),
...
correlation_plot <- ggcorr(X[,2:ncol(X)], palette = "RdYlGn", name = "rho",
       ...
       label_alpha = TRUE) + ggtitle('Red wine. Correlation of explanatory variables')
correlation_plot

# Saving it
ggsave(filename = paste('plots/Red wine correlation matrix.png'),
...
# load("draws/draws_red_normal 1.RData")
# load("draws/draws_red_normal 2.RData")
# load("draws/draws_red_normal 3.RData")
...
beta_posteriors_plot <- ggplot(betas_gathered, aes(beta_value, beta_name)) +
  ...
  ggtitle('Posterior distribution of betas for red wine with Normal response model')
beta_posteriors_plot

ggsave(filename = paste('plots/Red - Normal. Posterior distribution of betas Draw #3.png'),
...
wine_check <- c(123, 900, 1001) # these wines will be plotted
...
for(i in seq_along(ppd_plots)) ggsave(filename = paste('plots/Normal response and original variables',
                                                       'PPD of red wine #',wine_check[i],'.png'),
                                      plot = ppd_plots[[i]])
...
sulfur_dioxide_plot
ggsave('plots/Red wine. Free and total sulphur dioxide.png', plot = sulfur_dioxide_plot)
...
ggsave('plots/Red wine. Volatile and fixed acidity.png', plot = vol_fixed_acidity_plot)

# TRANSFORMED VARIABLE 3 - Log transformation of chlorides
chlorides_plot <- ggplot(X_plot, aes(x=`chlorides`, y=y)) +
  geom_point() + geom_smooth(method=lm)
chlorides_plot
ggsave('plots/Red wine. Chlorides and quality scatterplot.png', plot = chlorides_plot)

log_chlorides_plot <- ggplot(X_plot, aes(x=log(`chlorides`), y=y)) +
  geom_point() + geom_smooth(method=lm)
log_chlorides_plot
ggsave('plots/Red wine. Log Chlorides and quality scatterplot.png', plot = log_chlorides_plot)
...
correlation_plot_upd <- ggcorr(X_upd[,2:ncol(X_upd)], palette = "RdYlGn", name = "rho",
       label = TRUE, label_color = "black",
       label_alpha = TRUE) + ggtitle('Red wine. Correlation of explanatory variables (transformed)')
correlation_plot_upd

ggsave(filename = paste('plots/Red wine correlation matrix (transformed variables).png'),
...
# load("draws/draws_red_normal_transformed_vars.RData")
...
ggsave(filename = paste('plots/Red - Normal. Posterior distribution of betas (transformed variables).png'),
...
for(wine in seq_along(wine_check)) {
  ...
  ppd_plots_upd[[wine]] <- ggplot(ppd_tables_upd[[wine]], aes(x=wine_quality, y=probability)) +
    ...
    geom_vline(xintercept = y[wine_check[wine]], size = 2, color = '#AB200E') +
    scale_x_continuous(breaks = seq(0,10,1)) +
    ggtitle(paste("Posterior predictive distribution (normal response - transformed variables) for red wine number",
                  wine_check[wine]))
}

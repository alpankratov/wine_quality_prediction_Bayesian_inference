...
y <- d$quality
cat <- sort(unique(y))
# transforming y variable to matrix 4898 (number of observations) X 7 (number of quality scores)
# each row contains one 1 (TRUE) and six 0 (FALSE), TRUE represents falling in category
...
loglikelihood <- function(y, X, pars) {
  ...
  all_thetas <- exp(X %*% t(pars[,1:(npars-1)]))
...
# load("draws/draws_white_multinomial 1.RData")
# load("draws/draws_white_multinomial 2.RData")
# load("draws/draws_white_multinomial 3.RData")
...
for(i in seq_along(sample)) {
  ...
  betas_posterior_plots[[i]] <- ggplot(betas_gathered[[i]], aes(Beta_value, Beta_name)) +
    ...
    ggtitle(paste("White - Multinomial. Posterior distribution of beta coefficients for Quality Score",cat[i]))
}
# Plotting posterior distribution of betas
betas_posterior_plots

# Saving the plot
for(i in seq_along(betas_posterior_plots)) ggsave(filename = paste('plots/White - Multinomial. Posterior distribution,',
                                                                   'of betas for Quality Score',cat[i],' Draw #3.png'),
...
# we selected wines #1178, 2374 and 4619 to demostrate
...
for(wine in seq_along(wine_check)) {
  ...
  ppd_plots[[wine]] <- ggplot(data = posterior_thetas) +
    ...
    ggtitle(paste("Posterior predictive distribution (multinomial response) for white wine number",
                  wine_check[wine]))
}
...
for(i in seq_along(ppd_plots)) {
  ggsave(filename = paste('plots/PPD of white wine #',wine_check[i],'Multinomial response.png'),
         plot = ppd_plots[[i]])
}
